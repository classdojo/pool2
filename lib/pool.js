...
function Pool(opts) { // jshint maxcomplexity: 12, maxstatements: 45
    ...
    this.ensureMinimumInterval = validNum(opts, 'ensureMinimumInterval', Pool.defaults.bailAfter, true, true);
    ...
    this.acquiring = 0;
    this.pendingAcquiring = 0;
    this.pendingAcquiringTimeouts = [];
...
Pool.prototype.acquire = function (cb) {
    ...
    req.once('timeToAcquire', this.emit.bind(this, 'timeToAcquire'));
...
Pool.prototype.end = function (cb) {
    ...
    var closeResources = function () {
        ...
        this.pendingAcquiringTimeouts.forEach(clearTimeout);
...
Pool.prototype._ensureMinimum = function () {
    ...
    var n = this.min - (this.pool.count() + this.acquiring + this.pendingAcquiring);
    if (n <= 0) { return; }

    debug('Attempting to acquire minimum resources (cur=%d, min=%d)', this.pool.count(), this.min);
    while (n--) {

        if (this.ensureMinimumInterval > 0) {
            this.pendingAcquiring++;
            var timeout = setTimeout(function() {
                this.pendingAcquiring--;
                this._allocateResource();
            }.bind(this), n * this.ensureMinimumInterval);
            this.pendingAcquiringTimeouts.push(timeout);
        } else {
            this._allocateResource();
        }
    }
